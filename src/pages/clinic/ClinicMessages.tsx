...
export default function ClinicMessages() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data: refs } = await supabase.from("referrals").select("id, patient_name, referral_number").eq("clinic_id", profile.clinic_id);
      const list: Conv[] = [];
      for (const r of refs ?? []) {
    ...
    <div className="space-y-5">
      ...
      <Card className="shadow-card">
        <CardContent className="p-0 divide-y">
          ...
          {convs.map(c => (
            <Link key={c.referral_id} to={`/clinic/referrals/${c.referral_id}`} className="block px-5 py-3 hover:bg-secondary/40">
